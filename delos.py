...
\033[1;93mDeneyimsizler İçin Bir Penetrasyon Aracı v2\033[1;m 
...
Deneyimsizler İçin Bir Penetrasyon Aracı v3
\033[1;m      """)
...
# güvenlik zafiyet bilgi

def sqlinj():
	print("""
		SQL Injection
		SQL Injection, veri tabanına dayalı uygulamalara saldırmak
		için kullanılan bir atak tekniğidir; burada saldırgan
		SQL dili özelliklerinden faydalanarak standart uygulama
		ekranındaki ilgili alana yeni SQL ifadelerini ekler.
		(Örneğin saldırgan, veritabanı içeriğini kendisine aktarabilir).
		""")
def xssbilgi():
	print("""
		XSS (Cross Site Scripting)
		Siteler Arası Betik Çalıştırma (XSS), genellikle web uygulamalarında
		bulunan bir tür bilgisayar güvenlik açıklığıdır.
		XSS, diğer kullanıcılar tarafından görüntülenen
		web sayfalarına istemci taraflı kodun enjekte edilmesine imkan verir.

		""")
def csrfbilgi():
	print("""
		Cross Site Request Forgery
		Diğer Adlarıyla Xsrf, Csrf, Ve Cross Site Reference Forgery
		Browserlerın Javascript Çalıştırma Desteğiyle Kişilere
		İstenilenin Dışında Servera Komut Yollanmasını Sağlayan Kodlardır.
		""")
...
def baslangic():
	...
	if secim == "4":
				...
				logohack()
				zafiyetbilgi()
				secimzafiyetbilgi = raw_input("    " + kullanici + "" "\033[1;91m@KernelBlog:~$\033[1;m ")
				if secimzafiyetbilgi == "1":
						sqlinj()
						zafiyetbilgidevam()
				if secimzafiyetbilgi == "2":
			 			xssbilgi()
			 			zafiyetbilgidevam()
				if secimzafiyetbilgi == "3":
						csrfbilgi()
			 			zafiyetbilgidevam()
			 	if secimzafiyetbilgi == "4":
			 			os.system("clear")
			 			baslangic()
			 	if secimzafiyetbilgi == "5":
			 			print ("  Güle Güle " + " " + kullanici)
						time.sleep(0.5)
						os.system("clear")
						sys.exit()
	if secim == "Sistemi Güncelle" or secim == "sistemi güncelle" or secim == "5":
	...
	if secim == "çıkış" or secim == "Çıkış" or secim == "6":
	          print ("  Güle Güle " + " " + kullanici)
	          time.sleep(0.5)
	          sys.exit()
baslangic()
